...
try:
    ...
    # -------------------------------------------------------------
    # 2. ENQUIRY TABLE QUERY (Included phone_number and email)
    # -------------------------------------------------------------
    if is_admin:
        cursor.execute("""
            SELECT 
    ...
    for row in rows:
        ...
        if sample_status == 0:
            ...
            status_badge = (
                '<span class="badge bg-warning text-dark">New Enquiry</span>'
            )
            ...
            status_badge = (
                '<span class="badge bg-info text-dark">Sample Sent</span>'
            )
        ...
        created_date = created_at.strftime("%d-%m-%Y") if created_at else "N/A"
            ...
            <td class="text-right">
                <div class="d-flex justify-content-end gap-1">
                    <button class="table-action-btn edit-btn" 
                            title="Update"
                            data-id="{db_id}"
                            data-name="{customer or ''}"
                            data-phone="{phone_number or ''}"
                            data-email="{email or ''}"
                            data-quantity="{qty or 0}">
