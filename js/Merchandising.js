...
function populateModalDetails(btn) {
    ...
    document.getElementById('modalDeliveryDate').innerText = btn.getAttribute('data-deliverydate') || '--';
    document.getElementById('modalCreatedBy').innerText = btn.getAttribute('data-createdby') || '--';
    document.getElementById('modalRemarks').innerText = btn.getAttribute('data-remarks') || 'No remarks provided.';
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadSuppliers() {
        ...
        // Adding ?t= prevents browser caching of the API response
        fetch('../py/purchase/get_supplier_option.py?t=' + new Date().getTime())
            .then(response => response.json())
            .then(suppliers => {
    ...
    // 2. Initial load when DOM is ready
    loadSuppliers();

    // 3. Auto-refresh options whenever the browser tab gains focus
    window.addEventListener('focus', function () {
        loadSuppliers();
    });
